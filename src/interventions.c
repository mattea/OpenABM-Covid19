	...
	if( time_infected != UNKNOWN )
	{
		...
		for(int bucket = 0; bucket < N_NEWLY_INFECTED_STATES; bucket++)
		{
			if( indiv->infection_events->times[NEWLY_INFECTED_STATES[bucket]] > 0 )
			{
				...
				const double g = 1/8;
				const double b = 1/4;
				if( abs(time_infected - peak_time) > 9 )
				{
					indiv->lateral_flow_test_result = FALSE;
					break;
				}
				else if( time_infected <= peak_time )
				{
...
)
{
	...
	for( idx = 0; idx < n; idx++ )
		if( members[idx] != indiv->idx )
		{
			...
			if( params->lateral_flow_test_on_traced && ( index_token->index_status == POSITIVE_TEST ) )
		...
		token->index_status = index_token->index_status;
 		...
 		if( contact->traced_on_this_trace == FALSE )
 		{
 			if( contact->status != DEATH && !is_in_hospital( contact ) && !contact->infection_events->is_case )
 			{
				...
				if( params->lateral_flow_test_on_traced == TRUE )
					intervention_lateral_flow_test_order( model, contact, model->time + params->lateral_flow_test_order_wait );
